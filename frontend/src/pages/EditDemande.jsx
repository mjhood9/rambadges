...
const EditDemande = () => {
    ...
    useEffect(() => {
        ...
        const fetchDemande = async () => {
            try {
                ...
                const res = await fetch(`http://localhost:8080/api/demandes/${id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json"
                    }
                ...
                if (!res.ok) throw new Error("Failed to fetch demande");

                const data = await res.json();
                setOriginalDemande(data);
                setFormData((prev) => ({
                    ...prev,
                    ...data,
                    zones: data.zones || [],
                    ...
                    dateRecrutement: data.dateRecrutement ? new Date(data.dateRecrutement) : null,
                    dateExpirationPassport: data.dateExpirationPassport ? new Date(data.dateExpirationPassport) : null,
                    dateDebutPassport: data.dateDebutPassport ? new Date(data.dateDebutPassport) : null,
                    dateDebutPermis: data.dateDebutPermis ? new Date(data.dateDebutPermis) : null,
                }));
    ...
    const buildStatusRequest = (data) => {
        console.log("STATUS INPUT:", data);

        let status = data.status;
        let statusDirecteur = data.statusDirecteur;

        // 🔴 Correspondant reject → FORCE NULL via flag
        if (data.statusCorrespondant === "REJETEE") {
            return {
                status: "EN_ATTENTE",
                statusDirecteur: "EN_ATTENTE",
                clearStatusCorrespondant: true   // ✅ FORCE NULL
            };
        }

        // 🔴 Directeur reject
        if (statusDirecteur === "REJETEE") {
            return {
                status: "EN_ATTENTE",
                statusDirecteur: "EN_ATTENTE"
                // keep correspondant untouched
            };
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // 1. Fetch users & match current user
            ...
            // 3. MAIN UPDATE PAYLOAD (NO STATUS HERE)
            const payload = {
                ...formData,
            ...
            delete payload.statusDirecteur;
            delete payload.statusCorrespondant;
            // 4. Update demande data
            const res = await fetch(`http://localhost:8080/api/demandes/${id}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            // 5. STATUS LOGIC (USE ORIGINAL DEMANDE, NOT formData)
            console.log("ORIGINAL DEMANDE", originalDemande);
            const statusPayload = buildStatusRequest(originalDemande);
                        ...
                        <div className="form-row">
                            <div className="form-group">
                                ...
                                <div style={{ display: "flex", gap: "20px", alignItems: "flex-start" }}>
                                    ...
                                    {/* Existing signature */}
                                    {formData.signature && (
                                        <div className="existing-signature" style={{ flex: 1 }}>
                                            <label>Signature existante</label>
                    ...
                    <div className="step-form">
                        ...
                        <div className="form-row">
                            ...
                            <div className="form-group">
                                ...
                                {formData.cnieFile && (
                                    <p style={{ marginTop: "8px" }}>
                                        📎 {formData.cnieFile.name}
                                    </p>
                                )}

                                {/* EXISTING FILE */}
                                {formData.cnieFileUrl && (
                                    <p style={{ marginTop: "8px" }}>
                                        📄{" "}
                                        <a
                                            href={formData.cnieFileUrl}
                                            target="_blank"
                                            rel="noreferrer"
                                        >
                                            Voir fichier existant
                                        </a>
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="form-group">
                                ...
                                {/* EXISTING FILE */}
                                {formData.photoFileUrl && (
                                    <div style={{ marginTop: "8px" }}>
                                        <img
                                            src={formData.photoFileUrl}
                                            alt="Photo existante"
                                            style={{
                                                width: "120px",
                                                height: "120px",
                ...
                return (
                    <div className="step-form">
                        ...
                        <div className="files-container">
                            ...
                            {/* CNIE FILE */}
                            {(formData.cnieFile || formData.cnieFileUrl) && (
                                <div className="file-card">
                                    <a
                                        href={
                                            formData.cnieFile
                                                ? URL.createObjectURL(formData.cnieFile)
                                                : formData.cnieFileUrl
                                        }
                                        target="_blank"
                                        rel="noreferrer"
                                    ...
                                    <div className="file-footer">
                                        <p className="file-name">CNIE / Passport</p>
                                    </div>
                                </div>
                            )}

                            {/* PHOTO FILE */}
                            {(formData.photoFile || formData.photoFileUrl) && (
                                <div className="file-card">
                                    <a
                                        href={
                                            formData.photoFile
    ...
    return (
        <>
            <Helmet>
                <title>RAM Badges | Modifier Votre Demande</title>
            </Helmet>
