...
const CorrespondantDemande = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                // 2. LAISSEZ-PASSER BY DEMANDE
                const lpRequests = allDemandes.map((d) =>
                    axios.get(`http://localhost:8080/api/laissezpasser`, {
                        params: { demandeId: d.id },
                        headers: { Authorization: `Bearer ${token}` }
                    })
                        .then(res => {
                            console.log("📡 LP response for demande:", d.id, res.data);
                            return { id: d.id, data: res.data };
                        })
                ...
                const validationsData = allDemandes.filter(
                    (d) => d.statusCorrespondant === "EN_ATTENTE"
                );
    ...
    const filteredDemandes = demandes.filter((d) => {
        ...
        return matchSearch && matchDate && matchStatusDirecteur && matchStatusCorrespondant && matchDirection;
    });
    ...
    const filteredValidations = validations.filter((v) => {
        ...
        const matchDirection = selectedDirection
            ? v.direction === selectedDirection
            : true;

        return matchSearch && matchDate && matchDirection;
    });
...
// For "demandes" table
    ...
    return (
        <>
            <Helmet>
                <title>Correspondant de Sûreté RAM Badges | Les Demandes</title>
            </Helmet>
                            ...
                            <>
                                <table className="entite-table">
                                    ...
                                    <tr>
                                        ...
                                        <SortHeader label="Direction" sortKey="direction" sortConfig={sortValidations} onClick={handleSortValidations} />
                                        ...
                                        paginatedValidations.map(v => (
                                            <tr key={v.id}>
                                                ...
                                                <td>{v.secteur?.length
                                                    ? v.secteur.join(", ")
                                                    : "-"}</td>
                                                <td>{getStatusBadge(v.statusCorrespondant)}</td>
                                                <td style={{ textAlign: "right", display: "flex", justifyContent: "flex-end" }}> <div
                                                    className="detail-btn"
                                                    onClick={() => navigate(`/correspondantdesurete/demande/${v.id}`)}
                            ...
                            <>
                                <table className="entite-table">
                                    ...
                                    <tr>
                                        ...
                                        <SortHeader label="Direction" sortKey="direction" sortConfig={sortDemandes} onClick={handleSortDemandes} />
                                    ...
                                    {paginatedDemandes.length > 0 ? (
                                        paginatedDemandes.map((d) => {
                                            const lp = d.laissezPasser; // 👈 important
                                            ...
                                            <tr key={d.id}>
                                                ...
                                                <td style={{ textAlign: "right", display: "flex", justifyContent: "flex-end" }}> <div
                                                    className="detail-btn"
                                                    onClick={() => navigate(`/correspondantdesurete/demande/${d.id}`)}
                                                    style={{ cursor: "pointer" }}
                                                >
                                                    <i className="fa-regular fa-eye"></i>
                                                </div> </td>
...
export default CorrespondantDemande;